...
void loop()
{
  ...
  if (angle_changed)
  {
    myservo.write(lv_slider_get_value(ui_Slider_Servo));
    angle_changed = 0;
    lv_snprintf(buf, sizeof(buf), "Angle:%d", (int)lv_slider_get_value(ui_Slider_Servo));
    lv_label_set_text(ui_Slider_angle, buf);
    lv_snprintf(buf, sizeof(buf), "Angle:%d", (int)lv_arc_get_value(ui_Arc_Servo));
    lv_label_set_text(ui_arc_Angle, buf);
  }
}
